      ...
      <div class="current-product">
        ...
        <div class="wrapper_card center">
          <div class="current-product-card">
            <div class="current-product-card-type">COLLECTION</div>
            <hr class="under_type" />
            ...
            <div class="current-product-card-price">$<?=$data['price']?></div>
            <hr class="under_price" />
          </div>
        </div>
      </div>
      <div class="comments_wrapper center">
        <div class="comments">

        <?php
        include_once "./server/feedback.php";
        $comments = comments_all($connect, $product_id);
            if($comments){
                foreach ($comments as $comment){
                    echo "<div class='comment_item'><h2>{$comment['fio']}</h2><p>{$comment['text']}</p> <span>{$comment['date']}</span></div>";
                }
            }
        ?>
        </div>
        <div class="feedback">
        <form action="./server/feedback.php" method="POST" class="cart_details_forms" >
            <h2>Оставьте свой отзыв</h2>
            <input type="text" name="user_name" placeholder="Укажите ваше имя" required>
            <input type="text" name="user_feedback" placeholder="Краткий отзыв" required>
